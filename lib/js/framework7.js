    ...
    window.Framework7 = function (params) {
        ...
        ************   Swipe panels   ************
        ...
        app.initSwipePanels = function () {
            app.leftPanel = $('.panel.panel-left');
			app.rightPanel = $('.panel.panel-right');

            if (app.leftPanel.length === 0 && app.rightPanel.length === 0) return;
			...
			var targetClass;
			var targetPanel;
			var targetSide = app.params.swipePanel;//default swipe side

            function handleTouchStart(e) {
				//--- vivi ----
				targetClass = e.currentTarget.body.className;
				if ("with-panel-right-reveal" == targetClass) {
					targetPanel = app.rightPanel;
					targetSide = "right";
				} else if ("with-panel-left-reveal" == targetClass) {
					targetPanel = app.leftPanel;
					targetSide = "left";
				} else if(app.params.swipePanel && app.params.swipePanel != '') {
					targetSide = app.params.swipePanel;
					if ("left" == app.params.swipePanel) {
						targetPanel = app.leftPanel;
					} else if ("right" == app.params.swipePanel) {
						targetPanel = app.rightPanel;
					}
				} else {
					targetPanel = "";
				...
				var targetPanelClass = "";
				if (targetPanel) {
					targetPanelClass = targetPanel.attr("class");
				}

				//console.log("targetClass->"+targetClass+", targetSide->"+targetSide+", targetPanel->"+targetPanelClass);
				
				//------------
                if (!app.allowPanelOpen || !targetSide) return;
                if ($('.modal-in, .photo-browser-in').length > 0) return;

                touchesStart.x = e.type === 'touchstart' ? e.targetTouches[0].pageX : e.pageX;
                touchesStart.y = e.type === 'touchstart' ? e.targetTouches[0].pageY : e.pageY;
                if (app.params.swipePanelActiveArea) {
                    if (app.targetSide === 'left') {
                        if (touchesStart.x > app.params.swipePanelActiveArea) return;
                    }
                    if (app.targetSide === 'right') {
            ...
            function handleTouchMove(e) {
                ...
                if (!direction) {
                    ...
                    if (
						( direction === 'to-left' && app.rightPanel.hasClass('active') ) 
						  ||
                        ( direction === 'to-right' && app.leftPanel.hasClass('active') )
                    ) {
                        isTouched = false;
                        return;
                    }
                ...
                if (app.params.swipePanelNoFollow) {
                    ...
                    if (timeDiff < 300) {
                        if (direction === 'to-left') {
                            if (targetSide === 'right') app.openPanel(targetSide);
                            if (targetSide === 'left' && targetPanel.hasClass('active')) app.closePanel();
                        }
                        if (direction === 'to-right') {
							
                            if (targetSide === 'left') app.openPanel(targetSide);
                            if (targetSide === 'right' && targetPanel.hasClass('active')) app.closePanel();
                        }
                    }
                    isTouched = false;
                    isMoved = false;
                ...
                if (!isMoved) {
                    effect = targetPanel.hasClass('panel-cover') ? 'cover' : 'reveal';
					
                    targetPanel.show();
                    panelOverlay.show();
                    
					opened = targetPanel.hasClass('active');
					//console.log("effect->"+effect+", opened->"+opened);

                    panelWidth = targetPanel.width();
                    targetPanel.transition(0);
                    if (targetPanel.find('.' + app.params.viewClass).length > 0) {
                        if (app.sizeNavbars) app.sizeNavbars(targetPanel.find('.' + app.params.viewClass)[0]);
                    }					
                }
                ...
                if (targetSide === 'right') threshold = -threshold;
                ...
                if (effect === 'reveal') {
                    ...
                    targetPanel.transform('translate3d(' + translate + 'px,0,0)').transition(0);
                }
            ...
            function handleTouchEnd(e) {
                ...
                if (!opened) {
                    ...
                    if (translate === -panelWidth) {
                        ...
                        if (targetSide === 'left' && translate === panelWidth) action = 'reset';
                        else action = 'swap';
                    }
                    else {
                        action = 'reset';
                ...
                targetPanel.transition('').transform('');
        ...
        var Slider = function (container, params) {
            ...
            s.updatePagination = function () {
                if (s.paginationContainer && s.paginationContainer.length > 0) {
					...
					var textHTML = '<span class="slide-text"></span>';
                    s.paginationContainer.html(textHTML+bulletsHTML);
            ...
            s.updateClasses = function () {
                ...
                if (s.bullets && s.bullets.length > 0) {
					...
					var slideText = $(s.slides[s.activeSlideIndex]).find('span').text();
					$(s.container.find(".slide-text")[0]).text(slideText);
                }
            };
